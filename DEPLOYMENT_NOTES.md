...
先确认 Docker Compose 已安装，当前环境可正常执行 Compose 命令。
...
首次启动时，后端提示未配置邮件服务，验证码会输出到后端日志。为了测试真实邮件发送，在 `.env` 中配置 SMTP：
...
说明：`.env` 中已写入 `SMTP_PASSWORD`，本文档不记录明文密码。

修改 `.env` 后重启 Compose 服务，让后端重新读取环境变量：
...
后端请求日志显示 `/auth/send-code` 返回 `200`。

## 运行成功截图

截图占位：后续替换为浏览器打开 `http://localhost:3000` 后的运行成功截图。

```text
[运行成功截图占位]
```

![image-20260603163747961](/home/snemc/workspace/writeen_exam/test1/multica/assets/image-20260603163747961.png)
